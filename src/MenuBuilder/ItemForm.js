...
export default ({ value, onChange, onRemove, onDuplicate }) => {
  ...
  return (
    <form onSubmit={handleSubmit(onChange)}>
      <Stack>
        <Field r={register} name="label" label="Label" wrapperProps={{ style: { marginRight: '0.5em' } }} />
      ...
      <Toolbar>
        <Stack>
          ...
          <Button variant="secondary" onClick={onDuplicate}><Copy /></Button>
          <Button variant="secondary" onClick={onRemove}><Delete /></Button>
        </Stack>
      </Toolbar>
    </form>
  );
