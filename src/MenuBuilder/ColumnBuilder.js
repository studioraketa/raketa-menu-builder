  ...
  return (
    <Wrapper>
      <Toolbar>
        <Button
          variant='secondary'
          onClick={() => addColumn(state.columns, Factories.column())}
        >
          Add column
        </Button>
      </Toolbar>
      ...
      <ReactSortable
        list={state.columns}
        setList={(columns) => reorder(columns)}
        className='r-list'
        handle='.r-chandle'
        direction='vertical'
      >
          ...
          <Panel key={column.id} style={{ flex: 1, marginRight: '0.5em' }}>
            <PanelTitle>
              ...
              <Stack v='center'>
                <Button
                  variant='secondary'
                  onClick={() => duplicateColumn(state.columns, column)}
                >
                  <Copy />
                </Button>
                <Button
                  variant='secondary'
                  onClick={() => removeColumn(state.columns, column)}
                >
                  <Delete />
                </Button>
              </Stack>
